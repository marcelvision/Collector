            ...
            do
            {
                ...
                for (int i = 0; i < ltDeviceList.Count; i++)
                {
                    if (CSystem.MV_GIGE_DEVICE == ltDeviceList[i].nTLayerType)
                    {
                        ...
                        /*
                         * System.ArgumentException: ''GB2312' is not a supported encoding NameForm. For information on defining a custom encoding, see the documentation for the Encoding.RegisterProvider method. (Parameter 'NameForm')'
                        */
